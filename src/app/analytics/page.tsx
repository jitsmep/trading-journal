    ...
    return (
      <div className="space-y-6 animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-28 bg-zinc-900 rounded" />
          ))}
        </div>
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="border-b border-zinc-800 pb-5">
        ...
        <p className="text-zinc-400 text-sm mt-1">Deep dive into your performance patterns, psychological edges, and rules audit.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard
          ...
          icon={<Frown size={20} className="text-rose-400" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 shadow-xl">
          <h2 className="text-xl font-bold text-zinc-150 mb-1 tracking-tight">Mistake Frequency Index</h2>
          <p className="text-xs text-zinc-500 mb-4">Focus on eliminating these behavior leaks to boost performance.</p>
          
          {sortedMistakes.length > 0 ? (
            <div className="space-y-3">
              {sortedMistakes.map(([tag, count]) => (
                <div key={tag} className="flex justify-between items-center p-3 bg-zinc-950 rounded-lg border border-zinc-850">
                  <span className="text-rose-400 font-bold text-xs">{tag}</span>
                  <span className="text-zinc-300 font-semibold bg-zinc-900 px-3 py-1 border border-zinc-800 rounded text-xs">
            ...
            <div className="p-8 text-center text-zinc-500 italic bg-zinc-950/40 rounded-lg border border-dashed border-zinc-800">
        ...
        {/* Emotion Correlation */}
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 shadow-xl">
          <h2 className="text-xl font-bold text-zinc-150 mb-1 tracking-tight">Mindset vs Win Rate Correlation</h2>
          <p className="text-xs text-zinc-500 mb-4">Analysis of how your psychological state prior to entries affects outcomes.</p>
            ...
            <div className="space-y-5">
              ...
              <div>
                <div className="flex justify-between text-xs font-semibold mb-1.5">
                  <span className="text-zinc-350">Calm & Centered</span>
                  <span className="text-emerald-400 font-bold">{calmStats.winRate}% Win Rate <span className="text-zinc-500 font-normal">({calmStats.count} trades)</span></span>
                </div>
                <div className="w-full bg-zinc-950 rounded-full h-2 border border-zinc-850 overflow-hidden">
                  <div 
              ...
              <div>
                <div className="flex justify-between text-xs font-semibold mb-1.5">
                  <span className="text-zinc-355">Confident / High conviction</span>
                  <span className="text-emerald-400 font-bold">{confidentStats.winRate}% Win Rate <span className="text-zinc-500 font-normal">({confidentStats.count} trades)</span></span>
                </div>
                <div className="w-full bg-zinc-950 rounded-full h-2 border border-zinc-850 overflow-hidden">
                  <div 
                    className="bg-emerald-400 h-2 rounded-full transition-all duration-500" 
              ...
              <div>
                <div className="flex justify-between text-xs font-semibold mb-1.5">
                  <span className="text-zinc-350">Fear of Missing Out (FOMO) / Greed</span>
                  <span className="text-rose-450 font-bold">{fomoStats.winRate}% Win Rate <span className="text-zinc-500 font-normal">({fomoStats.count} trades)</span></span>
                </div>
                <div className="w-full bg-zinc-950 rounded-full h-2 border border-zinc-850 overflow-hidden">
                  <div 
